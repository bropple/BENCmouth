...
# Retina: the window carries the wordmark and 17 px type, and a 1x image on a
...
if command -v tiffutil >/dev/null 2>&1 && \
   [ -f assets/brand/dmg-background@2x.png ]; then
    if tiffutil -cathidpicheck assets/brand/dmg-background.png \
                ...
                -out /tmp/bm-dmg-background.tiff >/dev/null 2>&1; then
        BG=/tmp/bm-dmg-background.tiff
    fi
fi
