...
HEX=assets/icon/hex-256.png
OUT=assets/brand

# src/gui/bm_ui.c. The disk image uses these too.
BG='#0c1408'
BORDER='#2a3a1e'
DIM='#8aa878'
ACCENT='#78b946'
...
# ---------------------------------------------------------------- welcome
#
# Reads top to bottom: who made it, what it is, a rule, H. Hex, the licence.
...
"$MAGICK" -size 164x314 "xc:$BG" \
    ...
    -annotate +21+80 'formant speech synthesis' \
    -fill "$BORDER" -draw 'rectangle 22,100 142,100' \
    \( "$HEX" -resize 76x76 \) -geometry +44+140 -composite \
