...
#
# This is a speech synthesiser. It has never read a joystick and it never
# will, and on Windows the only thing GLFW's joystick support does for it is
# cost ten and a half seconds of startup.
#
...
if grep -q 'BENCmouth: no gamepads' "$J"; then
...
awk -v a="$ANCHOR" '
    ...
    { print }
    found && !done && $0 == "{" {
        print "    /* BENCmouth: no gamepads. See tools/no-gamepads.sh."
        print "     *"
...
grep -q 'BENCmouth: no gamepads' "$J" || {
